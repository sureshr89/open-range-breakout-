...
class DhanClient:
 ...
 def _post(self,path,payload):
  ...
  if time.monotonic()<self.cooldown_until: raise RuntimeError(f'Dhan cooldown active for {int(self.cooldown_until-time.monotonic())}s after HTTP 429')
...
class ORBEngine:
 def __init__(self,client_id,access_token,risk_per_trade=2250,max_daily_loss=5000,target_rr=2):
  self.dhan=DhanClient(client_id,access_token); self.risk=RiskState(float(risk_per_trade),float(max_daily_loss),float(target_rr)); self.state_file=Path(os.getenv('ORB_STATE_FILE','orb_state.json')); self.orb_file=Path(os.getenv('ORB_RANGE_FILE',f'orb_ranges_{today().isoformat()}.json')); self.state=self._load(); self.orb_cache=self._load_orb(); self.last_error=''; self.last_refresh=None; self.last_successful_quote=None; self.cache={'at':0,'df':None}
 def _load(self):
 ...
 def stock_scan(self):
  ...
  try:
   ...
   df=pd.DataFrame(rows); self.last_successful_quote=now(); self.last_refresh=now(); self.last_error=''; self.cache={'at':time.monotonic(),'df':df}; return df
 ...
 def unrealized_pnl(self):return 0
